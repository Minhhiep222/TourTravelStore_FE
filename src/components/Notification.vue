<template>
<div v-for="notification in notifications.slice().reverse()" :key="notification.id" class="bg-white">
  <div class="flex items-start p-4">
   <img alt="Shopee logo" class="w-12 h-12" height="50" src="https://image.pitchbook.com/w8elAV0qwPeHVx5Z0Xl2VJTEbst1489735146585_200x200" width="50"/>
   <div class="ml-4">
    ...
    <div class="flex mt-2 space-x-2">
     <img v-if="notification && notification.tour && notification.tour.images.length > 0"
                            :src="`http://127.0.0.1:8000/images/${notification.tour.images[0]?.image_url }`"
                            alt="Tour image"  class="w-24 h-24 object-cover" height="100" width="100"/>
                            <img v-if="notification && notification.tour && notification.tour.images.length > 0"
   ...
   <button @click="detailTour(notification.tour.encrypt_id)" class="ml-auto bg-gray-100 text-gray-700 px-4 py-2 rounded">
    Xem Chi Tiết
   </button>
  </div>
</div>
