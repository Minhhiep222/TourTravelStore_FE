<template>
  ...
  export default {
    ...
    methods: {
      async fetchTours() {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/tours/list`);
          // Lọc các tour có availability = 0
          const availableTours = response.data.tours.filter(tour => tour.availability === 0);
      ...
      async updateStatus(tourId, status) {
        try {
          const response = await axios.put(`http://127.0.0.1:8000/api/tours/${tourId}/status`, {
            status: status,
      ...
      async countTour() {
        try {
          const response = await axios.get(`http://127.0.0.1:8000/api/tours/total/count`);
