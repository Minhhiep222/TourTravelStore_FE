    ...
    <div class="p-4" id="app">
      <div class="bg-white p-4 rounded shadow-sm">
        ...
        <div v-for="(item, index) in favoriteTour" :key="index" class="flex items-center border-b py-4">
          ...
          <div class="w-1/5 text-center text-red-500 font-semibold">{{ item.price }}</div>
          <div class="w-1/5 text-center font-semibold">{{ item.duration }} ngày</div> <!-- Thời gian kéo dài -->
          <div class="w-1/5 text-center">
            <div class="text-red-500 cursor-pointer" @click="removeItem(item.id)">Xóa</div>
            <div class="text-sm text-gray-500 cursor-pointer" @click="detailTour(item.id)">
              Đặt tour
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-between items-center mt-4">
          <button @click="prevPage" :disabled="currentPage == 1" class="bg-gray-300 p-2 rounded">Trước</button>
          <span>Trang {{ currentPage }} / {{ totalPages }}</span>
          <button @click="nextPage" :disabled="currentPage == totalPages" class="bg-gray-300 p-2 rounded">Sau</button>
        </div>
...
export default {
  ...
  setup() {
    ...
    const getFavoritesTour = async (page = 1) => {
      ...
      try {
        const response = await axios.get('http://localhost:8000/api/getToursFavorite', {
          params: {
            user_id: userID,
            page: page
          }
        ...
        console.error("Failed to retrieve tours:", error.response ? error.response.data : error.message);
      }
    };
